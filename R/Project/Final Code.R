...
arm::standardize(cars_mod)

scale_fiesta <-  as.data.frame(scale(fiesta, center = T, scale = T))
fiesta_mod <- lm(price ~ ., scale_fiesta)
summary(fiesta_mod)

scale_focus <- as.data.frame(scale(focus, center = T, scale = T))
focus_mod <- lm(price ~ ., scale_focus)
summary(focus_mod)

scale_kuga <- as.data.frame(scale(kuga, center = T, scale = T))
kuga_mod <- lm(price ~ ., scale_kuga)
summary(kuga_mod)
