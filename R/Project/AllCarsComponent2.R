...
columnList <- c("price", "year", "transmission", "mileage", "fuelType", "tax", "mpg", "engineSize", "brand")
...
cars$brand <- as.numeric(cars$brand)
...
library(corrplot)
corrplot(res, tl.col = "black", tl.srt = 45, main = "Colour Correlation Plot")

modfit <- lm(price ~ ., data = cars)
modfit
